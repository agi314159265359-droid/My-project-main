...
public class OVRLipSyncContextMorphTarget : MonoBehaviour
{
    ...
    public void UpdateVisemeBlendShapes(float[] visemes)
    {
        for (int i = 0; i < visemeToBlendTargets.Length; i++)
        {
            if (visemeToBlendTargets[i] != -1)
            {
                skinnedMeshRenderer.SetBlendShapeWeight(
                    visemeToBlendTargets[i],
                    visemes[i] * 1.0f // Scale appropriately
                );
            }
        }
    }

    public void UpdateLaughterBlendShape(float laughter)
    {
        if (laughterBlendTarget != -1)
        {
            skinnedMeshRenderer.SetBlendShapeWeight(
                laughterBlendTarget,
                laughter * 1.0f // Scale appropriately
            );
        }
    }
