...
namespace FirstGearGames.LobbyAndWorld.Lobbies
{
    ...
    public class ReadyLobbyNetwork : LobbyNetwork
    {
        ...
        protected override bool OnCanStartRoom(RoomDetails roomDetails, NetworkObject startingPlayer, ref string failedReason, bool asServer)
        {
             ...
             * meaning players can join and leave as they wish. */
            if (roomDetails.IsStarted)
                return true;

            // ✅ ALWAYS ALLOW START - No ready check needed
            Debug.Log($"[ReadyLobbyNetwork] All players auto-ready, allowing start");
            return true;
        ...
        public void CleanupEmptyRoom(RoomDetails roomDetails)
        {
            _serverReadyPlayers.Remove(roomDetails);
        }
        ...
        [Server]
        public void CleanupPlayerReady(NetworkObject player)
        {

            if (player == null || player.Owner == null)
            {
                Debug.LogWarning($"[ReadyLobbyNetwork] CleanupPlayerReady called with null player");
                return;
            }

            Debug.Log($"[ReadyLobbyNetwork] Cleaning up ready state for Client {player.Owner.ClientId}");

            int clearedCount = 0;
            // Remove player from ALL rooms' ready lists
            foreach (var kvp in _serverReadyPlayers.ToList())
            {
                if (kvp.Value.Remove(player))
                {
                    clearedCount++;
                    Debug.Log($"[ReadyLobbyNetwork] Removed Client {player.Owner.ClientId} from room '{kvp.Key.Name}' ready list");
                }

                // Clean up empty hashsets
                if (kvp.Value.Count == 0)
                {
                    _serverReadyPlayers.Remove(kvp.Key);
                    Debug.Log($"[ReadyLobbyNetwork] Removed empty ready list for room '{kvp.Key.Name}'");
                }
            }

            Debug.Log($"[ReadyLobbyNetwork] Cleared ready state from {clearedCount} room(s)");
            // Remove player from ALL rooms' ready lists
            /* foreach (var kvp in _serverReadyPlayers.ToList())
             {
                 kvp.Value.Remove(player);

                 // Remove empty hashsets
                 if (kvp.Value.Count == 0)
                 {
                     _serverReadyPlayers.Remove(kvp.Key);
                 }
             ...
             Debug.Log($"[ReadyLobbyNetwork] Cleaned up ready state for player {player.Owner.ClientId}");
 */
        }
